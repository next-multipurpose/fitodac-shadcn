        ...
        grid never briefly holds two rows and shoves the panel off-center */}
				<AnimatePresence initial={false} mode="popLayout">
					{open ? (
						...
						>
							...
							>
								...
								<motion.div
									...
									// the middle in every direction instead of wiping
									// top-down.
									initial={
								...
								>
									{items.map((item, i) => {
										// Radial stagger: delay each item by its distance from
										// the grid center so the four corners animate together
										// and the open reads as center-out, not
										// corner-by-corner.
										const cols = 3
										...
										return (
											<button
												...
												// Static cell with hairline borders (no
												// animated fill) so the grid lines never
												// flicker as items stagger in. Only the
						...
						>
							{/* own `layout` counter-scales the label so it stays crisp
							while the button box morphs, instead of stretching with it */}
							<motion.span
								layout
