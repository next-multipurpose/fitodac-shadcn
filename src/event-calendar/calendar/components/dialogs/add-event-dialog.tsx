...
function formatEventTimeValue(value?: { hour: number; minute: number }) {
	if (!value) {
		return ""
	}
	...
	return formatTimeValue(date, "12", false)
}

function parseEventTimeValue(value: string) {
	const parsedDate = parseTimeValue(value, "12")
							...
							<FormField
								...
								render={({ field, fieldState }) => (
									<FormItem className="flex-1">
										...
										<FormControl>
											<TimeInput
												value={formatEventTimeValue(field.value)}
												onValueChange={(value) =>
													field.onChange(parseEventTimeValue(value))
												}
												format="12"
												data-invalid={fieldState.invalid}
											/>
