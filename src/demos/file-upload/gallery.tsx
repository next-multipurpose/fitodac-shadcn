...
export default function FileUploadGalleryDemo({
	maxFiles = 10,
			...
			{files.length > 0 && (
				<div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
					{files.map((fileItem) => (
						<div
							key={fileItem.id}
							className="group/item relative aspect-square overflow-hidden rounded-2xl border"
						>
							{isImage(fileItem.file) && fileItem.preview ? (
								<>
									...
									<img
										...
										className={cn(
											"size-full object-cover transition-all group-hover/item:scale-105",
											loadingImages[fileItem.id] !== false
												? "opacity-0"
												: "opacity-100"
										)}
							...
							{/* File Info */}
							<div className="absolute right-0 bottom-0 left-0 bg-black/70 p-2 text-white opacity-0 transition-opacity group-hover:opacity-100">
							...
							<>
								{isPreviewLoading && (
									<div className="absolute inset-0 flex items-center justify-center">
										<Loader2Icon className="size-8 text-white" />
									</div>
								)}
								<img
									src={selectedImage}
