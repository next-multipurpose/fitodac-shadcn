...
export default function TableProductsDemo() {
  const id = useId()
        ...
        <Table>
          <TableHeader>
            <TableRow className='hover:bg-transparent'>
              <TableHead>
                <Checkbox id={id} aria-label='Select all products' />
              </TableHead>
          ...
          <TableBody>
            {items.map(item => (
              <TableRow key={item.id} className='has-data-[state=checked]:bg-muted/50'>
                <TableCell>
                  <Checkbox id={`table-checkbox-${item.id}`} aria-label={`Select ${item.productName}`} />
                </TableCell>
                <TableCell>
