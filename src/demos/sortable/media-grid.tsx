...
export default function SortableMediaGridDemo() {
      ...
      >
        {items.map((item) => (
          <SortableItem key={item.id} value={item.id}>
            <div
              className={cn(
                "group relative cursor-pointer rounded-md border border-border bg-background p-3 transition-colors hover:bg-accent/50",
                getItemSize(item.type),
                "flex min-h-[100px] flex-col"
              )}
              onClick={() => {}}
            >
              <SortableItemHandle className="absolute end-1.5 top-2.5 z-10 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:text-foreground">
