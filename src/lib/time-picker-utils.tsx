...
export type InputTimePickerFormat = "12" | "24"

export function createTimeDate(date = new Date()) {
	return new Date(
		date.getFullYear(),
		date.getMonth(),
		date.getDate(),
...
export function formatTimeValue(
	...
	const hours =
		format === "12"
			? getValid12Hour(display12HourValue(date.getHours()))
	...
	if (format === "12") {
		return `${time} ${getPeriodFromDate(date)}`
	}
...
export function parseTimeValue(
	...
	if (format === "12") {
		...
		const [, hours, minutes, seconds = "00", period] = match
		const date = createTimeDate()

		date.setHours(
			convert12HourTo24Hour(parseInt(getValid12Hour(hours), 10), period.toUpperCase() as Period)
		)
		date.setMinutes(parseInt(getValidMinuteOrSecond(minutes), 10))
		date.setSeconds(parseInt(getValidMinuteOrSecond(seconds), 10))
	...
	date.setHours(parseInt(getValidHour(hours), 10))
	date.setMinutes(parseInt(getValidMinuteOrSecond(minutes), 10))
	date.setSeconds(parseInt(getValidMinuteOrSecond(seconds), 10))
